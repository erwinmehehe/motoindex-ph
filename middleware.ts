...
export function middleware(request: NextRequest) {
  ...
  if (process.env.NODE_ENV === "production" && isPrototypePath(pathname)) {
  ...
  if (failure && failure.count >= AUTH_MAX_FAILURES) {
    return deny("Too many authentication attempts.", 429, false, Math.max(1, Math.ceil((failure.resetAt - Date.now()) / 1000)));
  }
...
export const config = {
  matcher: [
    ...
    "/sellers/:path*", "/dealers/:path*", "/used-motorcycles/:path*", "/go/:path*",
