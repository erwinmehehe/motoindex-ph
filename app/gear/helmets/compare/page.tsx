...
export default async function HelmetComparePage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  <div className="section-head inline-head"><div><h2>Popular comparison guides</h2><p>Use these for category and brand research, then switch to the selectors above for exact model-to-model specs.</p></div></div>
  <div className="topic-grid">
    <article><h2>KYT vs LS2</h2><p>Verified price, type and feature coverage across the two brands.</p><div className="topic-action"><Link href="/gear/helmets/compare/kyt-vs-ls2">Open comparison →</Link></div></article>
    <article><h2>EVO vs Spyder</h2><p>Compare exact records instead of treating either brand as one product.</p><div className="topic-action"><Link href="/gear/helmets/compare/evo-vs-spyder">Open comparison →</Link></div></article>
    <article><h2>Full-face vs modular</h2><p>Compare the format tradeoffs before choosing exact models.</p><div className="topic-action"><Link href="/gear/helmets/compare/full-face-vs-modular">Open comparison →</Link></div></article>
  </div>
