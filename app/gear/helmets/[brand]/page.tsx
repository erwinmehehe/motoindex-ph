...
export default async function HelmetBrandPage({params}:{params:Promise<{brand:string}>}){
  ...
  const faqs: FaqItem[]=[
    {question:`How much is a ${h.brand} helmet in the Philippines?`,answer:minPrice&&maxPrice?`The ${h.brand} models in this guide have starting prices from ${php(minPrice)} to ${php(maxPrice)}. The exact price can change by graphic, visor bundle, size and seller, so open the source on the individual model before buying.`:`Some ${h.brand} model guides do not show a published price. Open the exact model and compare the current seller, size, graphic and included visor before buying.`},
  ...
  return <section className="page shell">
    ...
    {trackedCount>0?<>
      <div className="section-head inline-head"><div><h2>{h.brand} helmet models</h2><p>Browse the current {h.brand} model guides in one place. Models with a published Philippine price show it below; open any model for fit, visor, certification and seller checks.</p></div></div>
