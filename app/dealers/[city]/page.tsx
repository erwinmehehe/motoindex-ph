...
export async function generateMetadata({params}:{params:Promise<{city:string}>}):Promise<Metadata>{
  const {city}=await params; const list=publicDealersByCity(city); if(list.length < MIN_PUBLIC_DEALERS_PER_CITY)return {};
...
export default async function DealerCityPage({params}:{params:Promise<{city:string}>}){
  const {city}=await params; const list=publicDealersByCity(city); if(list.length < MIN_PUBLIC_DEALERS_PER_CITY)return notFound();
  const cityName=list[0].city; const province=list[0].province; const brands=[...new Set(list.flatMap(s=>s.brands))].sort();
  return <section className="page shell"><Breadcrumbs items={[{label:"Dealers",href:"/dealers"},{label:cityName}]} /><div className="page-head"><span className="entity-kicker">Checked dealer directory</span><h1>Motorcycle dealers in {cityName}</h1><p>Find checked dealer branches in {cityName}{province?`, ${province}`:""}. Confirm current stock, final cash price, registration fees and release timing directly with the branch before paying.</p></div>
    ...
    <div className="dealer-city-brands"><span>Brands in this directory</span><div>{brands.map(brand=><b key={brand}>{brand}</b>)}</div></div>
    <div className="section-head compact"><div><span className="section-kicker">Dealer profiles</span><h2>Checked branches in {cityName}</h2><p>Each record below has a reviewed dealer-verification source on file.</p></div></div>
    <div className="dealer-results">{list.map(s=><article className="dealer-result-card" key={s.slug}><div className="dealer-card-top"><span className="dealer-brand">{s.brands.join(" · ")}</span><span className="dealer-checked">Official listing checked</span></div><h3>{s.name}</h3><p>{s.addressLabel}</p><div className="dealer-card-meta">{s.phoneLabel?<span>{s.phoneLabel}</span>:null}<span>{s.categories.join(" · ")}</span></div><div className="dealer-card-actions"><Link href={`/sellers/${s.slug}`}>View dealer</Link>{s.phoneLabel?<a href={phoneHref(s.phoneLabel)}>Call branch</a>:null}</div></article>)}</div>
