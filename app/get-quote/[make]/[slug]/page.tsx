...
import { isPhilippineShoppingEligible } from "@/lib/marketScope";

export function generateStaticParams(){return motorcycles.filter(isPhilippineShoppingEligible).map(m=>({make:m.makeSlug,slug:m.slug}));}

export async function generateMetadata({params}:{params:Promise<{make:string;slug:string}>}):Promise<Metadata>{
  const {make,slug}=await params; const m=getModel(make,slug); if(!m || !isPhilippineShoppingEligible(m))return {};
  return pageMetadata({
...
export default async function QuotePage({params}:{params:Promise<{make:string;slug:string}>}){
  ...
  if(!m || !isPhilippineShoppingEligible(m))return notFound();
