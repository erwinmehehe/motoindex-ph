...
export async function generateMetadata({params}:{params:Promise<{make:string;slug:string}>}):Promise<Metadata>{
  ...
  return pageMetadata({
    ...
    description:`Request a current ${m.make} ${m.model} cash or installment quote from verified MotoIndex dealer partners serving your area in the Philippines.`,
    path:`/get-quote/${m.makeSlug}/${m.slug}`,
    index:isIndexableModel(m) && m.marketStatus!=="previous" && m.marketStatus!=="discontinued" && await hasQuoteEligibleDealerForBrand(m.make)
  });
}
    ...
    <div className="quote-grid">
      <div className="quote-summary">
        ...
        <ul className="checklist">
          ...
          <li>Matched only with verified dealer partners able to receive buyer requests</li>
          <li>Up to three relevant dealer partners can be matched for one request</li>
        </ul>
