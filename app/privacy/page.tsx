...
export default function PrivacyPage() {
  return <section className="page shell trust-page">
    <div className="page-head"><h1>Privacy at MotoIndex PH</h1><p>MotoIndex collects contact details only when you deliberately submit a dealer quote request, a dealer partner application, a configured price alert, or contact the site. Price-alert email collection stays unavailable unless the production delivery system is fully configured.</p></div>
    <div className="method-steps">
      ...
      <article><b>03</b><h2>Dealer sharing</h2><p>Submitting the form gives MotoIndex permission to share the request with a relevant verified dealer when a match exists. Approved dealer partners may receive a time-limited secure link containing the buyer details needed to respond to that request. The buyer may also receive a separate private quote-status link that shows only their own request and dealer responses; this link expires and should be kept private. If there is no verified match or approved delivery contact, MotoIndex does not represent the lead as delivered.</p></article>
      ...
      <article><b>06</b><h2>Price alerts</h2><p>When price alerts are enabled and you deliberately subscribe, MotoIndex stores your email address, motorcycle, target price, confirmation and unsubscribe tokens, subscription status, last price check, last observed price and alert-delivery timestamps. The subscription must be confirmed by email before threshold notifications can send, and every threshold email includes an unsubscribe link.</p></article>\n      <article><b>07</b><h2>Normal hosting logs may exist</h2><p>The hosting, CDN or security provider may process ordinary request information such as IP address, user agent, requested path and timestamps for delivery, abuse prevention and reliability. These logs are separate from the dealer-request fields stored by the MotoIndex lead form.</p></article>
      <article><b>08</b><h2>External sources and merchant links</h2><p>MotoIndex links to manufacturers, government agencies, retailers, marketplaces and original used-listing sources. Their privacy practices apply when you leave MotoIndex and visit those sites.</p></article>
      <article><b>09</b><h2>Commerce click measurement</h2><p>When the persistent commerce database is enabled, MotoIndex may store a merchant-offer identifier, product reference, merchant label and event timestamp when a merchant redirect is used. Commerce click records do not need the dealer-request form fields to measure whether a product link was opened.</p></article>
      <article><b>10</b><h2>Optional analytics</h2><p>Google Analytics or Plausible scripts load only when the deployment operator configures their environment variables. Before adding advertising pixels, new cookies, accounts or notification subscriptions, the operator should update the applicable consent and policy controls.</p></article>
      <article><b>11</b><h2>Corrections, privacy questions and deletion requests</h2><p>Use the published contact channel for a privacy question or a request concerning dealer information you submitted. MotoIndex also keeps a dedicated corrections process for published motorcycle and product data.</p><Link className="text-link" href="/corrections">Open corrections policy →</Link></article>
    </div>
  </section>;
}
