  ...
  return <section className="page shell">
    ...
    {listings.length>0 ? <>
      ...
      <div className="used-market-list">{listings.map(item=>{const model=getModelById(item.modelExternalId);return <article className="used-market-card" key={item.id}>
        <div><span>{item.modelYear} · {item.condition}</span><h2>{item.title}</h2><p>{item.location} · {item.mileageKm.toLocaleString("en-PH")} km · {item.sellerType}</p>{model&&<Link href={`/used-motorcycles/${model.makeSlug}/${model.slug}`}>More {model.model} listings →</Link>}</div>
    ...
    <div className="used-value-grid">{motorcycles.map(m=><Link key={m.id} href={`/used-motorcycles/${m.makeSlug}/${m.slug}`}><span>{m.make}</span><h2>{m.model}</h2><div><small>1 year</small><strong>{php(estimatedUsedValue(m,1))}</strong></div><div><small>3 years</small><strong>{php(estimatedUsedValue(m,3))}</strong></div><div><small>5 years</small><strong>{php(estimatedUsedValue(m,5))}</strong></div><b>Listings & estimator →</b></Link>)}</div>
  </section>;
}
