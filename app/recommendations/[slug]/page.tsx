...
function faqAnswer(question:string,models:Motorcycle[],guide:RecommendationGuide){
  ...
  if(!models.length)return "No current checked model set is available for this question.";
  const price=[...models].sort((a,b)=>observedMarketRange(a).from-observedMarketRange(b).from);
  const seat=[...models].sort((a,b)=>a.seatHeightMm-b.seatHeightMm);
  const weight=[...models].sort((a,b)=>a.curbWeightKg-b.curbWeightKg);
  const economy=models.filter(m=>m.fuelConsumptionKmL).sort((a,b)=>(b.fuelConsumptionKmL||0)-(a.fuelConsumptionKmL||0));
  ...
  if(lower.includes("how far")||lower.includes("theoretical fuel range"))return `Where both fields exist, MotoIndex calculates theoretical range as published km/L × tank capacity. It is a planning comparison only, not a real-world range estimate.`;
  if(lower.includes("how does motoindex rank motorcycles for daily commuting"))return `MotoIndex applies the same fixed profile to every model in this guide: 30-inch inseam, heavy stop-go traffic, 20 km daily round trip, solo-first use and the standard planning finance assumptions. The score rewards measurable fit, city-use and traffic factors; it is not a road-test verdict.`;
  ...
  if(lower.includes("what makes a motorcycle beginner-friendly"))return `MotoIndex uses lower curb weight, lower published seat height, manageable recorded output and ABS context as measurable starting factors. Training, controls familiarity, road conditions and an in-person fit check matter more than the list position.`;
  ...
  if(lower.includes("do beginners need abs"))return `ABS can be a valuable braking aid, but MotoIndex does not treat it as a substitute for training or correct braking technique. Check the exact trim because ABS equipment can vary by variant.`;
  ...
  if(lower.includes("verify before buying for expressway"))return `Verify the current tollway rules, the exact motorcycle's OR/CR classification and displacement entry, dealer documentation and any route-specific restrictions. MotoIndex does not infer legal access from a catalog displacement figure alone.`;
  ...
  if(lower.includes("all variants")||lower.includes("every variant"))return `No. This guide qualifies a model using its observed starting configuration, and equipment such as ABS can differ by trim. Always check the exact variant on the model page.`;
  if(lower.includes("dealer prices")||lower.includes("srp"))return `Not necessarily. MotoIndex keeps dated manufacturer, comparison-site and dealer observations separate and may show an observed range when sources disagree.`;
  ...
  if(lower.includes("horsepower per 100"))return `Horsepower per 100 kg can be a useful derived comparison for separating lightweight commuters from lighter performance models, but MotoIndex does not use it as the primary ranking factor on this page.`;
