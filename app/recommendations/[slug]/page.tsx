...
function positionReason(guide:RecommendationGuide,m:Motorcycle,index:number){
  ...
  if(guide.slug==="fuel-efficient-motorcycles-philippines")return `Position ${pos} follows the page rule because its published fuel-economy figure is ${m.fuelConsumptionKmL} km/L${pos===1?", the highest in the current qualifying set":""}.`;
...
function sectionSummary(title:string,models:Motorcycle[]){
  ...
  if(lower.includes("400cc motorcycles and expressway planning"))return `Every motorcycle in this guide has at least 400cc recorded displacement, but use the dedicated expressway-legal hub before planning a tollway trip. It separates the legal threshold from the buying comparison and highlights sub-400cc models that should not be rounded up from their model name.`;
  if(lower.includes("what to verify before an expressway trip"))return `Check the exact motorcycle's registered displacement and OR/CR details, the current rules for the tollway you will use, RFID/payment setup, tires, brakes, lights, mirrors and required rider documents. A 400cc+ catalog entry is only the first screening step.`;
  ...
  if(lower.includes("fuel-efficient")||lower.includes("fuel economy")||lower.includes("published fuel-economy")||lower.includes("published km/l"))return economy.length?`${economy[0].make} ${economy[0].model} has the highest published figure in this set at ${economy[0].fuelConsumptionKmL} km/L. Test methods can differ, so this is a planning reference rather than a controlled real-world ranking.`:"No current model in this section has a checked published fuel-economy figure.";
  ...
  if(lower.includes("city commuting"))return `For city use, start with the measurable constraint that matters most to you: published price, curb weight, seat height, transmission or published fuel economy. Then use the individual model pages to verify dimensions, variants and the current dealer price.`;
...
function faqAnswer(question:string,models:Motorcycle[],guide:RecommendationGuide){
  ...
  if(lower.includes("best published fuel")||lower.includes("highest published fuel"))return economy.length?`${economy[0].make} ${economy[0].model} has the highest published figure in this set at ${economy[0].fuelConsumptionKmL} km/L. Different test methods can limit direct real-world comparability.`:"No qualifying model currently has a checked published fuel-economy figure.";
  ...
  if(lower.includes("fuel-economy figures be used"))return `Use published km/L as a planning reference, not a guarantee. Test method, traffic, load, throttle use, tire pressure and maintenance can materially change real-world fuel consumption.`;
  ...
  if(lower.includes("single-channel")||lower.includes("dual-channel"))return `The recorded ABS wording can distinguish which wheel or channels are covered when that detail is available. Use the exact model/variant record rather than a generic ABS badge.`;
