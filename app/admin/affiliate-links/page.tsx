...
import { sourceBackedCommerceOffers } from "@/lib/commerceOffers";
...
export default async function AffiliateLinksAdmin(){
  ...
  const merchantOffersByProduct=new Map<string,typeof sourceBackedCommerceOffers>();
  for(const offer of sourceBackedCommerceOffers){
    const current=merchantOffersByProduct.get(offer.entityId)||[];
    current.push(offer);
    merchantOffersByProduct.set(offer.entityId,current);
  }
  ...
  const rows=allCatalogProducts().map(product=>{
    ...
    const merchantOffers=merchantOffersByProduct.get(product.id)||[];
    const merchantNames=[...new Set(merchantOffers.map(offer=>offer.sellerName))];
    return {
      ...product,
      dbLink:db?{
        url:db.url,
  ...
  return <section className="page shell">
    <div className="page-head">
      ...
      <p>Match MotoIndex helmets, tires and top boxes to checked merchants, then activate the exact Shopee or Involve Asia tracking URL. Runtime changes do not require a content rebuild.</p>
    ...
    <div className="note-box"><h2>Publication rule</h2><p>Only HTTPS links on approved Shopee or Involve Asia hosts can be activated. A checked source merchant can prove product availability without becoming an affiliate destination. An explicit database Disabled record blocks older JSON/environment fallbacks.</p></div>
