...
function TireSizeChart() {
  ...
  return <section className="page shell">
    ...
    <div className="helmet-brand-table"><div className="helmet-brand-row head"><span>Printed size</span><span>Catalog appearances</span><span>Use</span><span>Next step</span></div>{common.map(([size, count]) => {const sizeHub=tireSizeSeoHubs.find(h=>h.size===size);return <div className="helmet-brand-row" key={size}><strong>{size}</strong><span>{count}</span><span>Front and/or rear stock record</span><span><Link href={sizeHub?`/tires/${sizeHub.slug}`:"/tires"}>{sizeHub?"See motorcycles using this size →":"Match a motorcycle →"}</Link></span></div>;})}</div>
    <div className="section-head inline-head"><div><h2>Open a tire-size guide</h2></div></div><div className="topic-grid">{tireFamilyHubs.map(hub => <article key={hub.slug}><h2>{hub.shortName}</h2><p>{hub.description}</p><div className="topic-action"><Link href={`/tires/${hub.slug}`}>View tire sizes →</Link></div></article>)}{tireSizeSeoHubs.map(hub=><article key={hub.slug}><h2>{hub.size} tire index</h2><p>{hub.description}</p><div className="topic-action"><Link href={`/tires/${hub.slug}`}>See motorcycles using {hub.size} →</Link></div></article>)}</div>
