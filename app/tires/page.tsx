...
export default function TiresPage(){
  ...
  return <section className="page shell tire-master-page">
    ...
    <section id="size-chart" className="tire-master-section ui-page-section">
      <SectionHeader kicker="How to read the number" title="How motorcycle tire sizes work" description="A marking such as 110/80-14 combines tire width, aspect ratio and rim diameter. It does not tell you every fitment requirement." />
    ...
    <section id="families" className="tire-master-section ui-page-section">
      <SectionHeader kicker="Do not mix generations" title="Aerox, NMAX and Honda Click tire sizes" description="Popular model names span multiple generations. The exact generation matters because front and rear sizes can change." />
    ...
    <section id="stock-sizes" className="tire-master-section ui-page-section">
      <SectionHeader kicker="Exact motorcycles" title="Stock front and rear tire sizes" description="This is a compact preview. Use the Fitment Finder to search the complete current motorcycle catalog and open the exact model before ordering." aside={<Link href="/fitment">Search all fitment →</Link>} />
    ...
    <section id="common-sizes" className="tire-master-section ui-page-section">
      <SectionHeader kicker="Size index" title="Common stock motorcycle tire sizes" description="These are grouped from the motorcycles currently in the catalog. Use them for discovery only, then open the exact motorcycle before ordering." />
    ...
    <section id="products" className="tire-master-section ui-page-section">
      <SectionHeader kicker="Replacement tires" title="Verified motorcycle tire products" description="Product pages stay separate because each is a real tire family with its own manufacturer size range and use case." />
      <ProductGrid >{verifiedProducts.map(p=><ProductCard key={p.id} item={{entityId:p.id,href:`/tires/${p.brandSlug}/${p.slug}`,category:"Tire",brand:p.brand,model:p.model,meta:`${p.useCase} · ${p.knownSizes.length} listed sizes`,status:p.status,priceFromPhp:p.priceFromPhp}}/>)}</ProductGrid>
    </section>
