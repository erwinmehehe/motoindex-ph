...
const navGuides = recommendationGuides
    ...
    <header className="site-header">
      ...
      <div className="shell nav-wrap">
        ...
        <nav className="nav-links" aria-label="Primary navigation">
          ...
          <details className="nav-more nav-guides"><summary>Guides <span>⌄</span></summary><div className="nav-popover nav-popover-menu nav-popover-guides"><Link className="nav-popover-primary" href="/recommendations">Motorcycle buying guide</Link><Link href="/guides">Editorial guides</Link>{navGuides.map((guide) => <Link href={`/recommendations/${guide.slug}`} key={guide.slug}>{guide.title}</Link>)}</div></details>
        ...
        <details className="mobile-menu"><summary aria-label="Open navigation">Menu</summary><div className="mobile-menu-panel"><nav aria-label="Mobile navigation">
          ...
          {hasModels && <Link href="/finder">Finder</Link>}
          {hasComparisons && <Link href="/compare">Compare</Link>}
          <><strong className="mobile-menu-heading">Guides</strong><Link href="/recommendations">Motorcycle buying guide</Link><Link href="/guides">Editorial guides</Link>{navGuides.map((guide) => <Link href={`/recommendations/${guide.slug}`} key={guide.slug}>{guide.title}</Link>)}</>
