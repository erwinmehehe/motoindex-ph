...
export function CompareBuilder({ models }: { models: Motorcycle[] }) {
  ...
  const picker=(label:string,value:string,set:(v:string)=>void,optional=false)=><label><span>{label}</span><select value={value} onChange={e=>set(e.target.value)}><option value="">{optional?"None / two-way only":"Choose a motorcycle"}</option>{options.map(m=><option key={m.id} value={m.slug}>{m.make} {m.model}</option>)}</select>{value&&(()=>{const m=options.find(x=>x.slug===value);return m?<small>{observedMarketPriceLabel(m)} · {m.engineCc} cc · {m.seatHeightMm} mm seat</small>:null})()}</label>;
