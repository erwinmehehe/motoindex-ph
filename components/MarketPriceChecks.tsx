...
export function MarketPriceChecks({ model }: { model: Motorcycle }) {
  ...
  return <section className="market-checks">
    ...
    <div className="market-check-grid">
      {checks.length === 0 && <a href={baselineUrl} target="_blank" rel="noreferrer" className="baseline-price-source">
      ...
      {checks.map((row)=><a key={`${row.sourceName}-${row.sourceUrl}`} href={row.sourceUrl} target="_blank" rel="noreferrer">
