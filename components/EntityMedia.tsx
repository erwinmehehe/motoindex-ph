...
export function EntityMedia({ entityType, entityId, fallback, className, priority = false, sizes = "(max-width: 800px) 100vw, 42vw", linkHref, showCredit = true }: Props) {
  ...
  const image=<SafeEntityImage src={asset.src} fallbackSrc={asset.sourceImageUrl} alt={asset.alt} width={asset.width} height={asset.height} sizes={sizes} priority={priority} unoptimized={asset.src.endsWith(".svg")} />;
  return <div className={className || "entity-media"}>
    ...
    {showCredit&&<small className="entity-media-credit">{asset.sourceUrl ? <a href={asset.sourceUrl} target="_blank" rel="noreferrer">Image: {credit} ↗</a> : <>Image: {credit}</>}{asset.src.startsWith("/") && asset.sourceImageUrl ? " · local-first media" : asset.rightsStatus === "external-reference" ? " · external reference" : ` · ${asset.rightsStatus}`}</small>}
  </div>;
}
