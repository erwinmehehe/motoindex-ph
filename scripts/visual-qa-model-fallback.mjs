...
const port = 9444;
const profile = fs.mkdtempSync(path.join(os.tmpdir(), "motoindex-model-fallback-"));
  ...
  for (const width of widths) {
    ...
    for (const [name, pathname] of pages) {
      ...
      const audit = await evaluate(cdp.send, `(() => {
        ...
        const fallback=document.querySelector('.authority-media-fallback');
        ...
        const pathological=[...document.querySelectorAll('h1,h2,h3,strong')].filter(el=>{
          ...
          if(font<30)return false;
          const rect=el.getBoundingClientRect();
          const line=parseFloat(style.lineHeight)||font*1.1;
          const lines=rect.height/Math.max(line,1);
          return rect.width>0 && rect.width<Math.max(84,font*1.25) && lines>3.2;
        }).map(el=>({text:(el.textContent||'').trim().slice(0,70),width:Math.round(el.getBoundingClientRect().width),height:Math.round(el.getBoundingClientRect().height),font:parseFloat(getComputedStyle(el).fontSize||'0')})).slice(0,8);
        return {
      ...
      if (audit?.pathological?.length) failures.push(`${width}px ${pathname}: collapsed large text ${JSON.stringify(audit.pathological[0])}`);
      if (audit?.fallback) {
        const minFallback = width <= 430 ? 280 : width <= 1024 ? 360 : 430;
        const minTitle = width <= 430 ? 180 : 240;
        if ((audit.fallbackWidth || 0) < minFallback) failures.push(`${width}px ${pathname}: fallback panel collapsed to ${audit.fallbackWidth}px`);
