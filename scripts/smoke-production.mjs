...
async function request(path, options={}, expected=200){
  ...
  try{
    ...
    if(r.status!==expected)failures.push(`${path}: expected ${expected}, got ${r.status}`);
    if(r.status>=500){
      const body=await r.clone().text().catch(()=>"");
      if(/error\s*1102|worker exceeded resource limits/i.test(body))failures.push(`${path}: Cloudflare Worker exceeded resource limits (1102)`);
    }
    if(elapsed>8000)failures.push(`${path}: slow production response ${elapsed}ms`);
    return r;
  }catch(e){failures.push(`${path}: ${e instanceof Error?e.message:String(e)}`);return null;}
}
const get=(path,expected=200)=>request(path,{},expected);

const publicPaths=[
  "/","/motorcycles","/recommendations","/finder","/tires","/gear/helmets","/compare","/compare/three","/compare/three?bikes=aerox-v3,nmax-v3,adv-160","/search","/search?q=Click","/shortlist","/used-motorcycles","/deals",
  "/motorcycles/honda","/motorcycles/honda/click-160","/motorcycles/yamaha/aerox-v3","/motorcycles/electric","/motorcycles/electric/vinfast-evo","/motorcycles/electric/vinfast-feliz-ii","/motorcycles/electric/vinfast-viper",
  "/dealers","/dealers?brand=Honda","/dealers/manila","/dealers/san-fernando","/dealers/angeles-city","/dealers/cebu-city","/dealers/davao-city","/dealers/pampanga",
  "/used-motorcycles/repo","/used-motorcycles/buying-checklist","/maintenance","/ownership/registration-renewal",
  "/robots.txt","/llms.txt","/llms-full.txt","/deployment-info.json","/sitemap.xml","/sitemaps/motorcycles.xml","/sitemaps/gear.xml","/privacy"
];
await Promise.all(publicPaths.map(path=>get(path)));
...
if(deploymentInfo){
  try{
    ...
    if(base.hostname==="motoindexph.com"&&!new Set(["github-actions","cloudflare-workers-builds"]).has(info.provider))failures.push(`/deployment-info.json has unexpected provider ${info.provider||"missing"}`);
...
if(home){
  for(const [header,expected] of [["x-content-type-options","nosniff"],["x-frame-options","SAMEORIGIN"],["referrer-policy","strict-origin-when-cross-origin"]])if(home.headers.get(header)!==expected)failures.push(`/: missing or unexpected ${header} header`);
...
function isForbiddenIndexedPath(pathname){
  if(["/price-alerts","/sellers"].includes(pathname))return true;
...
for(const path of ["/price-alerts","/sellers","/motorcycles/honda/click-160/used-value"]){const r=await get(path,404);if(r&&!r.headers.get("x-robots-tag")?.includes("noindex"))failures.push(`${path}: prototype 404 missing X-Robots-Tag noindex`);}
for(const path of ["/sellers/demo-yamaha-dealer-a","/dealers/quezon-city"])await get(path,404);
for(const path of ["/sellers/desmark-honda-san-fernando-pampanga","/sellers/suzuki-motorcyclecity-san-fernando","/sellers/premiumbikes-san-fernando-pampanga","/sellers/yamaha-kservico-angeles","/sellers/yamaha-motor-ace-cebu-city","/sellers/yamaha-premio-davao-city","/sellers/desmark-honda-pardo-cebu","/sellers/suzuki-gud-moto-cebu","/sellers/kawasaki-des-strong-labangon","/sellers/desmark-honda-bajada-davao","/sellers/suzuki-trumph-davao","/sellers/kawasaki-emcor-bajada-davao"])await get(path);

for(const path of ["/api/leads","/api/price-alerts"]){
  await request(path,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email:"do-not-store@example.invalid",contact:"do-not-store"})},410);
}
...
if(models){
  ...
  }catch{failures.push("/api/models did not return valid JSON")}
}
const financeOk=await get("/api/finance?price=100000&down=20&months=36&rate=12");
if(financeOk){try{const body=await financeOk.json();if(!(body.estimatedMonthly>0))failures.push("/api/finance valid request returned no positive monthly estimate");}catch{failures.push("/api/finance valid request did not return valid JSON")}}
