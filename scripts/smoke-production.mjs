...
const publicPaths = [
  "/", "/motorcycles", "/robots.txt", "/sitemap.xml", "/sitemaps/motorcycles.xml", "/sitemaps/gear.xml", "/privacy",
...
const robots=await get("/robots.txt");if(robots){const body=await robots.text();for(const marker of ["Disallow: /admin/","Disallow: /api/","Sitemap:"])if(!body.includes(marker))failures.push(`/robots.txt missing ${marker}`);}

function isForbiddenIndexedPath(pathname){
  if (["/price-alerts","/deals","/sellers","/used-motorcycles"].includes(pathname)) return true;
  for(const prefix of ["/admin/","/api/","/get-quote/","/sellers/","/dealers/"]) if(pathname.startsWith(prefix)) return true;
...
for(const path of ["/sitemap.xml","/sitemaps/motorcycles.xml","/sitemaps/gear.xml","/sitemaps/commerce.xml"]){
  ...
  for(const raw of urls){
    try{const u=new URL(raw);if(isForbiddenIndexedPath(u.pathname))failures.push(`${path} leaks noindex/prototype route ${u.pathname}`)}catch{failures.push(`${path} contains invalid URL ${raw}`)}
  }
}
