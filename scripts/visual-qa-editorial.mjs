...
const fullPageNames = new Set(["recommendations", "ownership", "dealers", "helmets-hub", "tires-hub", "maintenance-hub", "commute-hub", "accessories-hub", "fitment-hub", "used-hub"]);
...
try {
  ...
  for (const width of widths) {
    ...
    for (const [name, pathname] of pages) {
      ...
      const audit = await evaluate(cdp.send, `(() => {
        ...
        const h1=document.querySelector('h1'); const safety=document.querySelector('#safety-efficiency');
        const grids=[...document.querySelectorAll('.product-grid,.fitment-list,.commute-rank-list,.guide-grid,.helmet-grid,.fitment-model-grid-v2,.used-value-grid-v2')].map(el=>({cls:String(el.className||''),count:el.children.length}));
        const largestGrid=grids.sort((a,b)=>b.count-a.count)[0]||{cls:'',count:0};
        const dealerSection=document.querySelector('.dealer-directory-section'); const ownershipCost=document.querySelector('.ownership-master-page #cost');
        const visibleProductCards=[...document.querySelectorAll('.product-card')].filter(visible).length;
        return {
          title:document.title, overflow:root.scrollWidth-root.clientWidth, broken,
          ...
          contentHeight:Math.round(root.scrollHeight), largestGrid, visibleProductCards,
          ...
          usedValueCards:document.querySelectorAll('.used-master-page .used-value-card').length,
          usedValueSearch:Boolean(document.querySelector('.used-master-page input[type="search"]')),
          usedValueBrandFilter:Boolean(document.querySelector('.used-master-page select'))
        };
      })()`);
      ...
      if (pathname === "/used-motorcycles") {
        if (!audit?.usedValueSearch || !audit?.usedValueBrandFilter) failures.push(`${width}px /used-motorcycles: bounded estimate search/filter controls are missing`);
        if ((audit?.usedValueCards || 0) > 18) failures.push(`${width}px /used-motorcycles: initial estimator renders too many model cards (${audit.usedValueCards})`);
        if ((audit?.usedValueCards || 0) < 8) failures.push(`${width}px /used-motorcycles: estimator discovery is unexpectedly sparse (${audit.usedValueCards})`);
      }
      ...
      if (fullPageNames.has(name)) {
        const metrics = await cdp.send("Page.getLayoutMetrics"); const size = metrics.cssContentSize || metrics.contentSize; const fullHeight = Math.min(Math.max(Math.ceil(size.height || 0), 1), 12000);
        const fullShot = await cdp.send("Page.captureScreenshot", { format: "png", fromSurface: true, captureBeyondViewport: true, clip: { x: 0, y: 0, width, height: fullHeight, scale: 1 } });
  ...
  proc.kill("SIGTERM"); await new Promise((resolve) => setTimeout(resolve, 250)); try { fs.rmSync(profile, { recursive: true, force: true }); } catch {}
}
