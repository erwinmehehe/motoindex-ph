...
const {loadTypeScript}=require("./load-typescript.cjs"); const ts=loadTypeScript();
const root=process.cwd();
const required=["lib/catalog.ts","components/ProductCard.tsx","components/CatalogExplorer.tsx","app/catalog/page.tsx","app/gear/helmets/[brand]/[product]/page.tsx","app/tires/[slug]/[product]/page.tsx","app/accessories/top-box/[product]/page.tsx","app/api/catalog/route.ts"];
...
const sourceFiles=[];
function walk(dir){for(const e of fs.readdirSync(dir,{withFileTypes:true})){const p=path.join(dir,e.name);if(e.isDirectory())walk(p);else if(/\.(ts|tsx)$/.test(e.name)&&!e.name.endsWith(".d.ts"))sourceFiles.push(p)}}
