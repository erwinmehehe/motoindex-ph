...
- **Honda Navi was intentionally held out of wave 3** until its manufacturer record could be re-verified. That source gap is now resolved in the next wave using Honda Philippines current catalog and official specification material.
...
- The existing /motorcycles/honda/navi canonical now owns price/spec/fuel-economy/beginner-commuter intent; no separate Navi price, specs or fuel-economy URL is created.
- The page receives a model-specific buyer brief, decision-authority content, current official-source metadata and CI regression protection.
